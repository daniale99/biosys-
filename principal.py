...
def main(page: ft.Page):
    ...
    def pagina_login():
        ...
        def btn_login_click(e):
            ...
            if validar_usuario(usuario, password):
                ...
                page.update()
                # Mostrar la página principal después de validar
                pagina_principal()
            else:
    ...
    def pagina_principal():
        ...
        def abrir_agregar_usuario(e):
            pagina_agregar_usuario()

        def abrir_consultar_usuarios(e):
            pagina_consultar_usuarios()

        btn_agregar = ft.ElevatedButton("Agregar Nuevo Usuario", on_click=abrir_agregar_usuario)
        btn_consultar = ft.ElevatedButton("Consultar Usuarios", on_click=abrir_consultar_usuarios)

        page.add(btn_agregar, btn_consultar)
        page.update()

    # Página para agregar nuevo usuario (sin cambios)
    def pagina_agregar_usuario():
        ...
        page.appbar = ft.AppBar(
            ...
            bgcolor="blue",
            leading=ft.Icon("arrow_back"),
            on_leading_click=lambda e: pagina_principal()
        )
    ...
    # Página para consultar usuarios (sin cambios)
    def pagina_consultar_usuarios():
        page.clean()
        ...
        try:
            ...
            lista = ft.Column()
            for r in registros:
                lista.controls.append(ft.Text(f"Usuario: {r['fields'].get('clave', '')}"))
            page.add(lista)
        except Exception as e:
